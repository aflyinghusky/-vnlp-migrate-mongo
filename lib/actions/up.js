...
module.exports = async (db, client) => {
  ...
  const migrateItem = async item => {
    try {
      ...
      await changelogCollection.delete({
        fileName,
      });
    ...
    try {
      await changelogCollection.findOneAndUpdate({
        fileName,
      }, useFileHash === true ? { fileName, fileHash, appliedAt } : { fileName, appliedAt });
